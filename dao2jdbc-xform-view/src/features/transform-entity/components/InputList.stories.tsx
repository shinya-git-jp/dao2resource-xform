...
import {EntityInputList} from './InputList';
...
} satisfies Meta<typeof EntityInputList>;
...
export const SelectGlobalSetting: Story = {
  ...
  play: async ({canvasElement}) => {
    ...
    waitFor(() => {
      ...
      expect(screen.getByText("エンティティ定義の外部キーを使用する：false")).toBeVisible();
    });
  }
}
...
export const SearchItem : Story = {
  ...
  play: async ({canvasElement}) => {
    ...
    await userEvent.click(screen.getByText("データベースID"));
    await userEvent.click(screen.getByRole("button", {name: "close"}));
    await await userEvent.click(screen.getByText("保存"));
    await waitFor(async () => {
      await expect(screen.getByText("エンティティ転換の条件一覧")).toBeVisible();
    });
    const searchTextbox = screen.getByLabelText("IDで検索する");
    await userEvent.type(searchTextbox, "testID");

    await waitFor(() => {
      expect(screen.queryByText("testID")).toBeVisible();
    });
