...
import { Button, Checkbox, Dialog, IconButton, InputLabel, List, ListItem, ListItemButton, ListItemText, Menu, MenuItem, Select, Tab, TextField, Toolbar, Typography } from '@mui/material';
import { TransformDialog } from './TransformDialog';
import { Close } from '@mui/icons-material';
import { EntityInputDialog } from './InputDialog';
import { SimpleTreeView } from '@mui/x-tree-view/SimpleTreeView';
import { v4 as uuidv4 } from 'uuid';
import { CustomTreeItem, EntityInputsSchema, readYaml, TitleTreeItem } from '../../../utils/utils';
import { useEntityInputListPageData } from '../hooks/useEntityInputListPageData';
...
export const EntityInputList: React.FC = () => {
  const {
    open,
    setOpen,
    openPreview,
  ...
  const handleClickOpenPreview = async () => {
    ...
    if (conditions.length == 0) {
      if (confirm("条件が空です。全てのエンティティが対象になります。続きますか?")) {
    ...
    try {
      ...
      try {
        for (const item of data) {
          ...
          } else if(item.databaseId != undefined) {
              tempItem = {
                id: uuidv4(),
                description: item.databaseId,
                type: "databaseId",
                setting: ""
              };
          } else {
  ...
  const filteredScripts = conditions.filter((script) => {
  ...
  const addItem = (condition: EntityCondition) => {
      const tempScript = conditions.find(c=> condition.id == c.id);
      if (tempScript) {
          tempScript.description = condition.description;
          tempScript.type = condition.type;
          tempScript.setting = condition.setting;
      } else {
          addCondition({
              id: condition.id,
    ...
    <Box textAlign="center" paddingBottom="50px" >
      <Box display="flex">
        <Box sx={{marginBottom: "10px", textAlign: "left"}}>
          <Typography variant="h6" textTransform="uppercase" fontWeight="bold" >GEF-JDBCのエンティティファイル変換ページ</Typography>
          <Typography gutterBottom textAlign={"left"} fontSize={16} sx={{color: "#999797"}}>
            GEF-DAOのエンティティからGEF-JDBCのエンティティファイルに変換するためのページです。
          </Typography>
        </Box>
        ...
        <TabContext value={tabIndex}>
          ...
          <Box display={"flex"} flexGrow={1}>
            <Box>
              ...
              <Typography textAlign={"left"} fontSize={16} sx={{color: "#999797"}}marginBottom={2}>
                このタブは変換するための条件を管理します。変換条件の中にエンティティIDや他の条件などを設定することができます。
              </Typography>
            </Box>
          ...
          />
            <Box textAlign="left" paddingTop="50px" sx={{border: "1px solid #c9cdd1", paddingLeft: 3, paddingRight: 3, paddingTop: 2, paddingBottom: 1, borderRadius: 5, marginBottom: 2}}>
                <Box display="flex">
                  <Typography variant="h6" fontWeight={"bold"} textAlign={"left"} marginBottom={1}>エンティティ変換の条件一覧</Typography>
                ...
                <Typography　variant="h6" fontWeight={"bold"} textAlign={"left"} marginBottom={1}></Typography>
                <TextField fullWidth label="IDで検索する" id="fullWidth" size="small" value={searchTerm} onChange={e=> {
                ...
                <>
                  <List sx={{ width: '100%', bgcolor: 'background.paper' }}>
                    {filteredScripts.map((item: any, index: number) => (
                        <ListItem
      ...
      <EntityInputDialog initialScript={condition} open={open} onClose={handleClose} onSubmit={addItem}/>
      ...
      >
        ...
        <TransformDialog conditions={conditions}/>
      </Dialog>
